...
@section('action-content')
  ...
  <div class="box-header">
    <div class="row">
        <div class="col-sm-8">
          <h3 class="box-title">Lista e Divizioneve</h3>
        </div>
        <div class="col-sm-4">
          <a class="btn btn-primary" href="{{ route('division.create') }}">Shto Divizion te ri</a>
        </div>
    </div>
  </div>
  <!-- /.box-header -->
  <div class="box-body">
      ...
      <form method="POST" action="{{ route('division.search') }}">
         ...
         @component('layouts.search', ['title' => 'Kerko'])
          @component('layouts.two-cols-search-row', ['items' => ['Emri Divizionit'], 
    ...
    <div id="example2_wrapper" class="dataTables_wrapper form-inline dt-bootstrap">
          ...
          <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
            ...
            @foreach ($divisions as $division)
                <tr role="row" class="odd">
                  <td>{{ $division->name }}</td>
                  <td>
                    <form class="row" method="POST" action="{{ route('division.destroy', ['id' => $division->id]) }}" onsubmit = "return confirm('A jeni i/e sigurt?')">
      ...
      <div class="row">
        <div class="col-sm-5">
          <div class="dataTables_info" id="example2_info" role="status" aria-live="polite">Rezultate 1 nga {{count($divisions)}} </div>
        </div>
        <div class="col-sm-7">
